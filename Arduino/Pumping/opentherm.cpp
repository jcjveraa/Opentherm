...
void OPENTHERM::setuppp_digitalRead() {
// 0 to 3
  for (int i = 0; i <= RX; i++) {
...
byte OPENTHERM::my_digitalRead(uint8_t pin)
{
  if(pin == RX) 
  {
    if (*portInputRegister(ports[pin]) & bitMasks[pin]) return LOW;
	  return HIGH;
  }
  ...
  // return !m_digitalReaddd(pin);
  // return m_digitalReaddd(pin);
}

void OPENTHERM::_timerISR() {
  if (_mode == MODE_LISTEN) {
    byte value = my_digitalRead(_pin);
...
String OPENTHERM::toFormattedString(OpenthermData &data) {
  ...
  if (data.type == OT_MSGTYPE_READ_DATA) {
    result.concat("ReadData");
  }
  else if (data.type == OT_MSGTYPE_READ_ACK) {
    result.concat("ReadAck");
  }
  else if (data.type == OT_MSGTYPE_WRITE_DATA) {
    result.concat("WriteData");
  }
  else if (data.type == OT_MSGTYPE_WRITE_ACK) {
    result.concat("WriteAck");
  }
  else if (data.type == OT_MSGTYPE_INVALID_DATA) {
    result.concat("InvalidData");
  }
  else if (data.type == OT_MSGTYPE_DATA_INVALID) {
    result.concat("DataInvalid");
  }
  else if (data.type == OT_MSGTYPE_UNKNOWN_DATAID) {
    result.concat("UnknownId");
  }
  else {
    result.concat(String(data.type, BIN));
  }
  result.concat(" ");
  result.concat(String(data.id));
  result.concat(" ");
  result.concat(String(data.valueHB, HEX));
  result.concat(" ");
  result.concat(String(data.valueLB, HEX));
